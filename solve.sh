...
if [ "$solver" = "-cadical" ]; then
    if [ "$mode" = "-cas" ]; then
        ...
        sms_cmd="timeout 1000000 smsg --vertices $n --print-stats True --triangle-vars $triangle_vars --non010 --all-graphs --dimacs $f"
        ...
        ./cadical-ks/build/cadical $f --proofsize 7168 -t $t | tee $f.log
    fi
