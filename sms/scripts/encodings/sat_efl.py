...
parser.add_argument('--chi3', type=int, help="Subgraph with minimum degree at least chi3 - 1; the subgraph contains all non-graph edges")
...
parser.add_argument('--vinzing', type=int, help="Max degree + 1")
parser.add_argument('--minDegreeAtmost', type=int, help="A vertex in the interseciton graph must have degree at most minDegreeAtmost")
...
if args.vinzing:
    maxDegree = args.vinzing - 1
    for v in V:
        counterImplementations.counterFunction([edge_contains_vertex(e,v) for e in E], countUpto=maxDegree, vPool=vpool,
                    clauses=constraints, atMost=maxDegree, type=DEFAULT_COUNTER)
...
if args.minDegreeAtmost:
    ...
    for e1 in E:
        counters = counterImplementations.counterFunction([var_edge_intersection_graph(e1,e2) for e2 in E if e1 != e2], countUpto=max_count, vPool=vpool,
                    clauses=constraints, type=DEFAULT_COUNTER)
        constraints.append([-selected[e1], -counters[args.minDegreeAtmost + 1 - 1 ]])

        counters2 = counterImplementations.counterFunction([edge_contains_vertex(e1,v) for v in V], countUpto=3, vPool=vpool,
                    clauses=constraints, type=DEFAULT_COUNTER)
        constraints.append([-selected[e1], counters2[3 - 1]]) # at least three vertices in that edge
