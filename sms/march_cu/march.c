...
//      printf("   -c <file>     parse cube from <file>    (default:             no cube)\n");
      ...
      printf("   -a <int>      minimum number of assigned variables in each cube  (default: %4.0f, dynamic depth)\n", (float) assignmentCutoff);
      ...
      printf("   -f <int>      selected scoring function for lookahead (default: %d,      no limit)\n", scoringFunction);
...
//      printf("   -v            more verbose output\n\n");
  ...
  for (i = 2; i < argc; i++) {
    ...
    if (strcmp(argv[i], "-a"  ) == 0) { assignmentCutoff    = strtoul (argv[i+1], NULL, 10); }
    if (strcmp(argv[i], "-l"  ) == 0) { cubeLimit  = strtoul (argv[i+1], NULL, 10); }
    if (strcmp(argv[i], "-L"  ) == 0) { hardLimit  = strtoul (argv[i+1], NULL, 10); }
    if (strcmp(argv[i], "-f"  ) == 0) { scoringFunction = strtoul (argv[i+1], NULL, 10); }
...
//    printf("c down fraction = %.3f and down exponent = %.3f\n", (float) fraction, (float) downexp);
  ...
  if (!cut_var && !cut_depth && !assignmentCutoff) dynamic = 1;
...
*/
	...
	if (initSolver ())
	{
     ...
     if (assignmentCutoff)
        {
          int fixedVariables = (nrofvars - freevars);

          printf("c Number of free variables: %i\n", freevars);
          printf("c Fixed variables: %i\n", fixedVariables);
          printf("c Number of variables: %i\n", nrofvars);
          int cut_var_tmp =  assignmentCutoff - fixedVariables; // TODO
          printf("c cut_var = %i\n", cut_var_tmp);
          if (cut_var_tmp <= 0)
          {
            printf("Already more fixed variables than the cutoff\n");
            exit(1);
          }
          cut_var = cut_var_tmp;
        }
