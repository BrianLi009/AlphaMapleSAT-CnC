...
int main(int argc, char const **argv)
{
    ...
    output_opts.add_options()
      ...
      ("simplify", po::value<std::string>(&config.simlifiedCNFFile), "Simplify the CNF formula and write it to the given file. (Directly after prerun)")
      ("learned-clauses", po::value<std::string>(&config.learnedClausesFile), "Write learned clauses with size 5 and unit clauses to given file")
      ("simplify-redundant-max-size", po::value<int>(&config.simplifiedRedundantMaxSize), "The maximal size of redundant clauses which should stil be added to the simplified file");
    ...
    perf_opts.add_options()
      ...
      ("lookahead", po::bool_switch(&config.lookahead), "Use lookahead on edge variables")
      ("lookaheadAll", po::bool_switch(&config.lookaheadAll), "Use lookahead on all variables")
      ("lookaheadHeuristic", po::value<int>(&config.lookaheadHeuristic)->default_value(1), "Specify which lookahead heuristic is used. (For details see cadicalLookahead.cpp)")
      ...
      // ("assignment-cutoff-all", po::value<int>(&config.assignmentCutoffAll), "Same as assignment-cutoff, but over all variables and not only edge variables")
      ...
      ("cube2solve", po::value<std::vector<int>>()->multitoken()->composing()->notifier([&rangeCubesTemp](const std::vector<int> &values)
          ...
          else EXIT_UNWANTED_STATE}),
              "A range of cubes which should be solved given by the first and last cube (for external parallelization; see also --cubes)")
