...
{
  if (inLookaheadState)
    return true; // skip propagation in lookahead state
...
bool GraphSolver::solve()
{
  ...
  if (!config.simlifiedCNFFile.empty())
  {
    simplify();
    printf("Simplified CNF file was written to file\n");
    return true;
  }
  ...
  {
    printLearnedClauses();
    printf("Learned clauses were written to file\n");
    return true;
  }
