...
void App::print_usage (bool all) {
  ...
  if (!all) { // Print only a short list of common options.
    printf ("  -h             print this short list of common options\n"
            ...
            "  --no-print     disable solution printing\n"
#ifndef QUIET
    ...
    printf (
        ...
        "  --no-print     disable solution printing (only show satisfiability)\n"
        "\n"
...
int App::main (int argc, char **argv) {
  ...
  bool use_nauty = false;
  bool use_master = false;
  bool use_ortho = false;

  bool print_statement = false;
  bool no_print = false;  // Add option to disable solution printing
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp (argv[i], "-h") || !strcmp (argv[i], "--help") ||
      ...
      if (++i == argc)
        APPERR ("argument to '--partition' missing");
      else if (partition_size != 0)
        APPERR ("multiple argument '--partition %d' and '--partition %s'", partition_size, argv[i]);
      else if (!parse_int_str (argv[i], partition_size))
        APPERR ("invalid argument in '--partition %s'", argv[i]);
      else if (partition_size < 0 || partition_size > 40)
        APPERR ("invalid partition size");
      else {
        partition_size = stoi(argv[i]);
        std::cout << "c partition size = " << partition_size << endl;
      }
    } else if (!strcmp (argv[i], "--unembeddable-check")) {
      if (++i == argc)
        APPERR ("argument to '--unembeddable-check' missing");
      else if (unembeddable_check != -1)
    ...
    } else if (!strcmp (argv[i], "--print-statement") ||
               !strcmp (argv[i], "--print-statement=true") ||
               !strcmp (argv[i], "--print-statement=1")) {
      print_statement = true;
      std::cout << "c Debug statements enabled" << std::endl;
    } else if (!strcmp (argv[i], "--no-print-statement") ||
               !strcmp (argv[i], "--print-statement=false") ||
               !strcmp (argv[i], "--print-statement=0")) {
      print_statement = false;
      std::cout << "c Debug statements disabled" << std::endl;
    } else if (!strcmp (argv[i], "--nauty")) {
      use_nauty = true;
    } else if (!strcmp (argv[i], "--master")) {
      use_master = true;
      std::cout << "c Using master graph for subgraph checking" << std::endl;
    } else if (!strcmp (argv[i], "--ortho")) {
      use_ortho = true;
      std::cout << "c Using orthogonality checking" << std::endl;
    } else if (!strcmp (argv[i], "--no-print") ||
               !strcmp (argv[i], "--no-print=true") ||
               !strcmp (argv[i], "--no-print=1")) {
      no_print = true;
      std::cout << "c Solution printing disabled" << std::endl;
    } else if (!strcmp (argv[i], "--print") ||
               !strcmp (argv[i], "--no-print=false") ||
               !strcmp (argv[i], "--no-print=0")) {
      no_print = false;
      std::cout << "c Solution printing enabled" << std::endl;
    }
#ifndef __WIN32
  ...
  if (incremental) {
    ...
    std::unique_ptr<SymmetryBreaker> sb(new SymmetryBreaker(solver, order, unembeddable_check, partition_size));
    sb->set_use_master_graph(use_master);
    sb->set_print_statement(print_statement);
    sb->set_use_orthogonality(use_ortho);
    sb->set_no_print(no_print);
