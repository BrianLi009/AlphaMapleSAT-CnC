...
def mpisubmit(executor, jobs):
    fs = [executor.submit(cube, *job) for job in jobs]
    for future in as_completed(fs):
        mpisubmit(executor, future.result())
...
def main(order, file_name_solve, m, solving_mode="other", cubing_mode="march", numMCTS=2, cutoff='d', cutoffv=5, solveaftercube='True', timeout=2147483647):
    ...
    # MPI
    top = [[file_name_solve, "N", 0, m, order, numMCTS, timeout, cutoff, cutoffv, d, cubing_mode]]
    with MPIPoolExecutor() as executor:
        nworkers = executor.num_workers
        print(f"Running with {nworkers} workers")
        mpisubmit(executor, top)
